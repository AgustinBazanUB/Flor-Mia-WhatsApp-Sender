...
async function sendToWhatsApp<T extends keyof InboxResponseMap>(
  ...
  if (!response?.ok || response.data === undefined) {
    ...
    const code = Object.values(ERROR_CODES).includes(remoteCode as never)
      ? remoteCode as (typeof ERROR_CODES)[keyof typeof ERROR_CODES]
      : ERROR_CODES.internal;
    throw new ExtensionError(code, response?.error?.message || "WhatsApp Web no respondió al Inbox.", {
