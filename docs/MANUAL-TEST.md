...
La matriz de aceptación formal de la versión 0.9.1 RC está en [`ACCEPTANCE-TESTS.md`](ACCEPTANCE-TESTS.md). Registrar allí evidencia PASS/FAIL; ninguno de esos casos se considera ejecutado contra WhatsApp real por el solo hecho de pasar los tests automatizados.
...
3. Al terminar la carga, pulsar **Comprobar** desde la Web-App o ejecutar diagnóstico desde el popup y confirmar que un preflight funcional vuelve a **🟢 VERDE**.
...
6. Cambiar a **JSON**, pulsar **Copiar JSON** y confirmar `reportSchemaVersion: 1`, versión `0.9.1`, incidente, checkpoint, compatibilidad, Last Known Good, candidates y trace.
