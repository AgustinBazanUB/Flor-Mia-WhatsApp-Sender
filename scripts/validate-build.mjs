...
if (sourceManifest.version !== "0.9.5.5") throw new Error("La release esperada para el extractor estructurado es 0.9.5.5.");
...
if (!contactHtml.includes("PHONE_UNRESOLVED") || !contactHtml.includes("Chats abiertos") || !contactHtml.includes("codex-json")) {
  throw new Error("El build no contiene la UX de extracción phone-first y diagnóstico 0.9.5.5.");
}
...
if (!whatsappContent.includes("label-scoped-phone-first-no-chat-opening")) {
  throw new Error("El Content Script no contiene la estrategia 0.9.5.5 label-scoped/phone-first.");
}
if (!backgroundWorker.includes("WAWebCollections") || !backgroundWorker.includes("WAWebApiContact") || !backgroundWorker.includes("main-world-label-store+local-lid-map")) {
  throw new Error("El build no contiene el resolver estructurado local de etiquetas/LID de 0.9.5.5.");
}
...
if (!backgroundWorker.includes("virtualized-lid-hydration") || !whatsappContent.includes("flormia_contact_export_lid_resolve_v1")) {
  throw new Error("El build no contiene la hidratación virtualizada LID→teléfono de 0.9.5.5.");
}
